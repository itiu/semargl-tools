...
public class MessageParser
{
    ...
    synchronized public List<OntoFunction> functionsFromMessage(String message)
    {
    ...
    synchronized public Triple getArgumentByPredicate(List<Triple> args, String argName)
    {
        Triple result = null;
    ...
    synchronized public List<String> split(String tripletsLine)
    {
        ...
        if (tripletsLine != null && tripletsLine.trim().length() > 0)
        {
            ...
            for (int i = 0; i < tripletsLine.length(); i++)
            {
                ...
                if (c != ' ')
                {
                    ...
                    if (isProcessNeeded)
                    { // если один из разделителей, то не пох
    ...
    synchronized public void searchParams(char ch, int delim_number)
    {
        prev_delimiter = ch;
        switch (delim_number)
        {
